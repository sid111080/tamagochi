...
class Pet {
  ...
  /// Размер аватара: плавный рост на каждый уровень (0.68 → 1.40, потолок).
  ///
  /// Стадии (Малыш/Ребёнок/…) — дискретны и меняются только на 3-м и 5-м
  /// уровнях, поэтому связывать размер со стадией нельзя: на 2-м уровне
  /// питомец был бы такого же размера, как на 1-м, и рост не виден.
  /// Считаем от уровня: каждый уровень даёт заметный шаг (~18%). Для видов
  /// с одним эмодзи (панда 🐼, единорог 🦄) размер — главный сигнал роста.
  /// Аватар обёрнут в AnimatedContainer — при новом уровне плавно «выдышит».
  double get sizeScale => (0.68 + (level - 1) * 0.12).clamp(0.68, 1.40);
